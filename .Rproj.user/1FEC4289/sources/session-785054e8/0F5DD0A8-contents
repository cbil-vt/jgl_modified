...
JGL <-
           ...
           truncate = 1e-5)
  {
    ...
    {
      if (K == 2)
        #use bi-conditional screening rule to identify block structure exactly
      {
        crit1 = list()
        for (k in 1:K) {
          crit1[[k]] =  abs(S[[k]]) * weights[k] > lam1 + lam2
        }
      ...
      if (K > 2)
        #use sufficient screening rule to identify larger-grained block structure
      {
        crit1 = list()
        for (k in 1:K) {
    ...
    if (penalty == "group")
    {
      ...
      for (k in 1:K) {
        tempsum = tempsum + (pmax(weights[k] * abs(S[[k]]) - lam1, 0)) ^ 2
      }
    ...
    {
      theta.unconnected = admm.iters.unconnected(
        Yu,
        lambda1 = lam1.unconnected,
        lambda2 = lam2.unconnected,
        penalty = penalty,
        rho = rho,
        weights = weights,
        maxiter = maxiter,
    ...
    if (length(blocklist) > 0) {
      for (i in 1:length(blocklist)) {
        ...
        # initialize lambdas:
        lam1.bl = penalty.as.matrix(lam1.bl, dim(Ybl[[1]])[2], penalize.diagonal =
                                      penalize.diagonal)
        if (penalty == "fused") {
          lam2.bl = penalty.as.matrix(lam2.bl, dim(Ybl[[1]])[2], penalize.diagonal =
                                        TRUE)
        }
        if (penalty == "group") {
          lam2.bl = penalty.as.matrix(lam2.bl, dim(Ybl[[1]])[2], penalize.diagonal =
    ...
    if (!return.whole.theta)
    {
      out = list(
        theta = theta,
        theta.unconnected = theta.unconnected,
        connected = connected
      )
    }
    if (return.whole.theta)
...
print.jgl <-
  function(x, ...)
  {
    ...
    for (k in 1:K) {
      sharededges = sharededges + (x$theta[[k]] != 0)
    }
    ...
    norm = c()
    for (k in 1:K) {
      norm[k] = sum(abs(x$theta[[k]])) - sum(abs(diag(x$theta[[k]])))
    }
    cat("L1 norm of off-diagonal elements of classes' Thetas: ",
        norm,
        "\n")
  }
